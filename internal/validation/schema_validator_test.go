...
func TestYAMLValidationSuccess(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			yaml:    "../../tools/test_data/valid_simple.yaml",
			schema:  "../../docs/ralphy-inputs.schema.json",
			wantErr: false,
		},
		{
			name:    "Invalid YAML - missing required field",
			yaml:    "../../tools/test_data/invalid_missing_required.yaml",
			schema:  "../../docs/ralphy-inputs.schema.json",
			wantErr: true,
		},
		{
			name:    "Invalid YAML - wrong type",
			yaml:    "../../tools/test_data/invalid_wrong_type.yaml",
			schema:  "../../docs/ralphy-inputs.schema.json",
			wantErr: true,
		},
		{
...
func TestLoadAndCompileSchema(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			schema:    "../../tools/test_data/invalid_schema.json",
			wantErr:   true,
			errString: "failed to compile schema",
		},
	}
...
func TestValidationReportStructure(t *testing.T) {
	...
	if !ok {
		t.Fatal("Missing or invalid summary field in validation report")
	}
	...
	if summary["validation_type"] != "yaml_syntax" {
		t.Errorf("Expected validation_type 'yaml_syntax', got %v", summary["validation_type"])
	}
...
func TestYAMLValidationReportContent(t *testing.T) {
	...
	details, ok := summary["validation_details"].(map[string]interface{})
	if !ok {
		t.Fatal("Missing or invalid validation_details in report")
	}
	...
	if fixes, ok := details["fixes_applied"]; ok && fixes != nil {
		if len(fixes.([]interface{})) == 0 {
...
func TestSchemaValidationReportStructure(t *testing.T) {
	...
	summary, ok := report["summary"].(map[string]interface{})
	if !ok {
		t.Fatal("Missing or invalid summary field in schema validation report")
	}
	...
	if summary["validation_type"] != "json_schema" {
		t.Errorf("Expected validation_type 'json_schema', got %v", summary["validation_type"])
	}

	result, ok := summary["validation_result"].(map[string]interface{})
	if !ok {
		t.Fatal("Missing or invalid validation_result in report")
	}
	...
	if !ok {
		t.Fatal("Missing or invalid errors in validation_result")
	}
...
func TestSchemaValidationReportContent(t *testing.T) {
	...
	compliance, ok := summary["schema_compliance"].(map[string]interface{})
	if !ok {
		t.Fatal("Missing or invalid schema_compliance in report")
	}

	requiredFields, ok := compliance["required_fields_present"].([]interface{})
	if !ok {
		t.Fatal("Missing or invalid required_fields_present in compliance")
	}
	...
	tasksCount, ok := compliance["tasks_count"].(float64)
	if !ok {
		t.Fatal("Missing or invalid tasks_count in compliance")
	}
	...
	tasksValid, ok := compliance["tasks_valid"].(float64)
	if !ok {
		t.Fatal("Missing or invalid tasks_valid in compliance")
	}
