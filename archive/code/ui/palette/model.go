...
func (m Model) View() string {
	...
	// Calculate modal dimensions with safe bounds
	modalWidth := min(max(m.width-4, 20), 80)
	modalHeight := min(max(m.height-4, 5), 25)
...
func (m Model) renderCommandList(width, height int) string {
	...
	// Truncate to fit height (safe bounds check)
