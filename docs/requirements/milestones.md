...
1) M1 — Repo Init & CLI scaffold (1-2 days)
...
2) M2 — Requirements Parser + Template-based Plan Mode (2-4 days)
...
3) M3 — SQLite knowledge DB + caching (2-3 days)
...
4) M4 — Basic Validator and Review (non-AI) (2 days)
...
5) M5 — Ralphy Executor (MVP shell) + vendor bundling (3 days)
...
6) M6 — Build Mode: Commit-per-task + pre-flight checks (2-3 days)
...
7) M7 — Context Optimization (minimal viable features) (4-6 days)
...
8) M8 — Hybrid & AI flow hooks (optional for MVP or staged next sprint) (2-3 weeks)
...
9) M9 — Knowledge export/import, team sharing, and simple CI templates (2 days)
...
- High-priority MVP path: M1 → M2 → M3 → M4 → M5 → M6. These give an end-to-end Plan Mode (code-gen) and a safe Build Mode with vendored Ralphy dry-run and commit-per-task simulation.
- Mid-term: M7 (context optimization) yields the 80%+ token reduction target.
- Longer: M8 (AI flows) and M9 (team sharing + CI) come afterwards.
...
3. Start implementation: I can scaffold the CLI (M1) next — tell me to proceed.
